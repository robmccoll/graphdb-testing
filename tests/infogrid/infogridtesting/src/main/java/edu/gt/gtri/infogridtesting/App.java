package edu.gt.gtri.infogridtesting;
...
public class App 
{
  ...
  public static void main( String[] args )
  {
	...
	vertices = new MeshObject[(int)nv];

	for(long v = 0; v < nv; v++) {
	  vertices[(int)v] = mb.getMeshBaseLifecycleManager().createMeshObject();
	  //veritces[(int)v].setPropertyValue(vid, IntegerValue.create(v));
	}
	for(int v = 0; v < nv; v++) {
	  for(int e = (int)off[v]; e < off[v+1]; e++) {
	    try {
	      vertices[v].relate(vertices[(int)ind[e]]);
	    } catch(RelatedAlreadyException error) {
	      // Silently ignore
	    }
	  }
	}
