...
public class SimplePageRankComputation extends BasicComputation<LongWritable,
  ...
  public static final int MAX_SUPERSTEPS = 80;
  /** Logger */
  ...
  private static String DIFF_AGG = "diff";
...
//    if (getSuperstep() >= 1) {
      ...
      double oldVertexValue = vertex.getValue().get();

      DoubleWritable vertexValue =
          new DoubleWritable((0.15f / getTotalNumVertices()) + 0.85f * sum);
      vertex.setValue(vertexValue);

      double absDiff = Math.abs(oldVertexValue - vertexValue.get());

      aggregate(MAX_AGG, vertexValue);
      aggregate(MIN_AGG, vertexValue);
      aggregate(SUM_AGG, new LongWritable(1));
      aggregate(DIFF_AGG, new DoubleWritable(absDiff));

      LOG.info(vertex.getId() + ": PageRank=" + vertexValue +
          " max=" + getAggregatedValue(MAX_AGG) +
          " min=" + getAggregatedValue(MIN_AGG) +
	  " diff=" + getAggregatedValue(DIFF_AGG));
    }

    DoubleWritable totalDiff = getAggregatedValue(DIFF_AGG);
    if (getSuperstep() < 2)
      totalDiff.set(1.0);

    if (getSuperstep() < MAX_SUPERSTEPS && totalDiff.get() > 1e-8) {
  ...
  public static class SimplePageRankMasterCompute extends
    ...
    public void initialize() throws InstantiationException,
      ...
      registerAggregator(DIFF_AGG, DoubleSumAggregator.class);
