...
/*! \brief Simple implementation of an array that automatically grows as
    ...
    with no elements allocated.  If a size is passed to the constructor, the
    array is set to be exactly the size passed (both size and reserved
    memory).  A difference from the STL vector is that the elements in the
    array are NOT initialized.  When a dynamic_array automatically resizes
    ...
    stl::vector.  A difference from the STL vector is that reserve() and
    resize(), when changing the size of the data array, set the new size of
    the data array to exactly the size specified by the user.  This conserves
    memory by not allocating up to the next smallest power of 2.
*/
/****************************************************************************/
template <typename T>
class dynamic_array {
public:
  ...
  inline dynamic_array(size_type size = 0);
  ...
  /// The destructor.
  ~dynamic_array() { if (alloc_size > 0) free(data); }
  ...
  void pop_back(size_type size = 1) { mt_incr(num_elements, 0 - size); }
...
dynamic_array<T>::dynamic_array(size_type size) :
  num_elements(size), alloc_size(size)
{
  data = alloc_size > 0 ? (T*) malloc(alloc_size * sizeof(T)) : (T*) NULL;
}
...
T& dynamic_array<T>::operator[](size_type i)
{
  ...
  {
    std::cerr << "error: index " << i << " out of bounds" << std::endl;
    exit(1);
  }
#endif
...
{
  assert(new_size > 0);
  size_type old_alloc_size = alloc_size;
  alloc_size = new_size;
