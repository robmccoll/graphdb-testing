...
public:
  typedef typename property_traits<IdMap>::key_type key_type;
  typedef typename property_traits<IdMap>::value_type hash_key_type;
  typedef E value_type;
  map_property_map(xmt_hash_table<hash_key_type, E>& m, IdMap id) 
       : data(m), id_map(id) {}

  value_type& operator[] (key_type k) const { return data[get(id_map,k)]; }

protected:
  template <typename E2, typename IdMap2>
  friend E2 get(const map_property_map<E2, IdMap2>& pa,
                const typename property_traits<IdMap2>::key_type& k);

  template <typename E2, typename IdMap2, typename K2>
  friend void put(const map_property_map<E2, IdMap2>& pa,
                  const typename property_traits<IdMap2>::key_type& k,
                  const K2& v);

  xmt_hash_table<hash_key_type, E>& data;
  IdMap id_map;
};
