...
public:
  ...
  static void runloop2(const size_t startat,
                       const size_t stopat, void* arg)
  {
    ...
    for (size_t i = startat; i < stopat; i++)
    {
      ...
      bool added = ht.insert(key, key).second;
...
#ifdef DEBUG
    std::cerr << "n: " << n << ", m: " << m << ", tolerance: " << tolerance
              << std::endl;
#endif
  ...
  // 13 * scale * num_edges_per_phase in each phase.
    ...
    if (static_cast<unsigned long>(num_edges_added) != htab.size())
    {
      std::cerr << "error: num_edges: " << num_edges_added << ", ht.sz: "
                << htab.size() << std::endl;
      exit(1);
    }
                         ...
                         xmt_hash_table<int64_t, int64_t>& ht)
  {
...
#ifdef DEBUG
    std::cout << "create_edges_loop(): num_edges_per_phase: "
              << num_edges_per_phase << std::endl;
#endif
    ...
    {
#ifdef DEBUG
      std::cout << "create_edges_loop(): iter: " << num_edges_added
                << std::endl;
#endif

      mt_drand48(rand_val_size, rv_p);
                     ...
                     xmt_hash_table<int64_t, int64_t>& htab, int n)
  {
    ...
    for (size_type i = 0; i < num_edges_added; i++)
    {
      ...
      bool added = htab.insert(key, key).second;
      if (added)
      {
...
#ifdef DEBUG
    std::cout << "create_edges(): toadd: " << num_edges_to_be_added
              << ", next: " << next << ", faliures: " << failures << std::endl;
#endif
  }
  ...
  void rmat_stats(graph_adapter& g)
  {
    ...
    std::cout << "num_a: " << num_a << " (" << num_a / (double) m << "%)"
              << std::endl
              << "num_b: " << num_b << " (" << num_b / (double) m << "%)"
              << std::endl
              << "num_c: " << num_c << " (" << num_c / (double) m << "%)"
              << std::endl
              << "num_d: " << num_d << " (" << num_d / (double) m << "%)"
              << std::endl;
  }
...
/*! \brief A scale-free graph generator based on the R-MAT (recursive matrix)
    ...
    b, c, and d, where a + b + c + d = 1. Some noise is added at each stage of
    the recursion, and the a, b, c, d values are renormalized.

    The generator is called with two required parameters (scale and
    edge_factor) and four optional parameters (a, b, c, and d). The number of
    vertices is set to 2^scale, and the number of edges to edge_factor *
    2^scale. The probabilities for the four partitions are expressed by the
    parameters a, b, c, and d.
    
    As stated in the paper by Chakrabarti, Zhan and Faloutsos:

      Typically, a ≥ b, a ≥ c, a ≥ d.
        * The partitions a and d represent separate groups of nodes which
          correspond to communities (say, soccer and automobile enthusiasts).
        * The partitions b and c are the cross-links between these two groups;
          edges there would denote friends with separate interests.
        * The recursive nature of the partitions means that we automatically
          get sub-communities within existing communities (say, motorcycle and
          car enthusiasts within the automobile group).
      The third bullet results in 'communities within communities'.

    Here are two different suggested settings for the probabilities.

       Nice settings:  a = 0.45;  b = 0.15;  c = 0.15;  d = 0.25;
      Nasty settings:  a = 0.57;  b = 0.19;  c = 0.19;  d = 0.05;
...
{
  detail::gen_rmat_graph<graph_adapter> grg(scale, edge_factor, a, b, c, d);
  grg.run(g);
}

}

#undef GEN_HAMILTONIAN_CYCLE
