...
template <typename graph, typename visitor>
void visit_adj_inner_ind(const size_t startat,
...
template <typename graph, typename visitor>
void visit_adj_inner(const size_t startat, const size_t endat,
                     void* arg)
{
  typedef typename graph_traits<graph>::vertex_descriptor vertex_t;
  typedef typename graph_traits<graph>::vertex_iterator vertex_iterator;
                        ...
                        int degreeThreshold = 5000)
{
  ...
  for (int i = 0; i < num_big; i++)             // In serial.
  {
    ...
    visit_adj_inner<graph, visitor>((size_t) indicesOfBigs[i],
...
template <typename graph, typename visitor>
void visit_adj_chunked_inner(const size_t startat,
                             const size_t endat, void* arg)
{
  typedef typename graph_traits<graph>::vertex_descriptor vertex_t;
  typedef typename graph_traits<graph>::size_type size_type;
