...
void merge_sort(T* array, long size, Comparator comp, long block_size = 1024)
{
  ...
  for ( ; array_size < (size * 2) && array_size < split_point; array_size *= 2)
  {
    ...
    for (long j = 0; j < size; j += array_size)
    {
      // Get the starting and ending positions in the left and right halves.
      long left_pos = j;
      long right_pos = j + (array_size / 2);
      long left_end = size > right_pos ? right_pos : size;
      long right_end = size > j + array_size ? j + array_size : size;
      long b_pos = left_pos;
      ...
      if (left_pos < left_end)
      {
        ...
        #pragma mta noalias *a, *b
        #pragma mta assert nodep
        for (long k = left_pos; k < left_end; ++k)
        {
      ...
      else
      {
        ...
        #pragma mta noalias *a, *b
        #pragma mta assert nodep
        for (long k = right_pos; k < right_end; ++k)
        {
  ...
  long* left_start = (long*) malloc(num_blocks * sizeof(long));
  long* left_end = (long*) malloc(num_blocks * sizeof(long));
  long* right_start = (long*) malloc(num_blocks * sizeof(long));
  long* right_end = (long*) malloc(num_blocks * sizeof(long));
  long* extra_left_start = (long*) malloc(num_blocks / 2 * sizeof(long));
  long* extra_left_end = (long*) malloc(num_blocks / 2 * sizeof(long));
  long* extra_right_start = (long*) malloc(num_blocks / 2 * sizeof(long));
  long* extra_right_end = (long*) malloc(num_blocks / 2 * sizeof(long));
  ...
  {
    long half_array_size = array_size / 2;
    long blocks_in_half_array = half_array_size / block_size;
    long num_half_arrays = (size + half_array_size - 1) / half_array_size;
    long half_num_half_arrays = num_half_arrays / 2;
    long num_array_blocks = half_num_half_arrays * blocks_in_half_array;
    ...
    for (long i = 0; i < num_array_blocks; ++i)
    {
    ...
    for (long i = 1; i < num_array_blocks; ++i)
    {
      ...
      else
      {
        ...
        long left_pivot = left_start[i];
        long right_first = i / blocks_in_half_array * array_size +
                           half_array_size;
        long right_last = size < right_first + half_array_size ?
        ...
        if (comp(a[left_pivot - 1], a[right_first]))
        {
        ...
        else if (comp(a[right_last], a[left_pivot]))
        {
          ++right_last;
        }
        else
        {
          ...
          while (right_last - right_first > 1)
          {
            ...
            if (comp(a[right_center], a[left_pivot]))
            {
              right_first = right_center;
            }
            else if (comp(a[left_pivot], a[right_center]))
            {
    ...
    for (long i = 0; i < num_array_blocks; ++i)
    {
      ...
      long num_new_subblocks =
        (right_end[i] - right_start[i] + block_size - 1) / block_size - 1;

      if (num_new_subblocks > 0)
      {
        long extra_pos = mt_incr(num_extra_blocks, num_new_subblocks);
        ...
        for (long j = 0; j < num_new_subblocks; ++j)
        {
          long right_pivot = extra_right_end[extra_pos + j];
          long left_first = extra_left_start[extra_pos + j];
          long left_last = left_end[i] - 1;
          long left_center;
          ...
          if (comp(a[right_pivot - 1], a[left_first]))
          {
          ...
          else if (comp(a[left_last], a[right_pivot]))
          {
            ++left_last;
          }
          else
          {
            ...
            while (left_last - left_first > 1)
            {
              ...
              if (comp(a[left_center], a[right_pivot]))
              {
                left_first = left_center;
              }
              else if (comp(a[right_pivot], a[left_center]))
              {
    ...
    {
      // Set the ranges.
      long left_pos = left_start[i];
      long right_pos = right_start[i];
      long b_pos = left_start[i] + right_start[i] -
    ...
    #pragma mta assert nodep
    for (long i = 0; i < num_extra_blocks; ++i)
    {
      // Set the ranges.
      long left_pos = extra_left_start[i];
      long right_pos = extra_right_start[i];
      long b_pos = extra_left_start[i] + extra_right_start[i] -
    ...
    {
      #pragma mta assert nodep
      for (long i = half_num_half_arrays * array_size; i < size; ++i)
      {
        b[i] = a[i];
      }
    }
...
/// Allows the merge sort to be called using std::less<T> as the default
/// comparator.
template <typename T>
inline
void merge_sort(T* array, long size, long block_size = 1024)
{
  merge_sort(array, size, std::less<T>(), block_size);
}
...
void merge_sort(T* array, long size, T2* array2, Comparator comp,
...
inline
void merge_sort(T* array, long size, T2* array2, long block_size = 1024)
{
  merge_sort(array, size, array2, std::less<T>(), block_size);
}
